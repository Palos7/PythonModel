...
def model_xgb(train, test):    
    params = {'booster':'gbtree','objective': 'binary:logistic','eval_metric': 'auc','eta': 0.027,  'silent': 1,      'max_depth': 9,
              'min_child_weight': 2.3,   'gamma':0,    'lambda':1,    'colsample_bylevel':0.7,'subsample':0.9,    'scale_pos_weight':1}    
  
    dtrain=xgb.DMatrix(train.drop(['User_id','Coupon_id','label','Date_received'],axis=1),label=train['label'])    
...
big_train.fillna(-1,inplace=True) 
test.fillna(-1,inplace=True)   
result = model_xgb(big_train, test)
result.to_csv('C:/Users/27Palos/Desktop/Xgb.csv', index=False, header=None) 
